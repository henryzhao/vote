...
public class FileUpload {
     ...
     * Extra
     */
    private static String extraFile;
    ...
    static {
        ...
        try {
            ...
            extraFile = prop.getProperty("extra.file");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static FileItem parse(MultipartFile file) throws IOException {
        FileType type = FileFormat.judge(file.getInputStream());
        String format = type.toString().toLowerCase();
        if (type == FileType.UNKNOWN){
            String [] unknown = file.getOriginalFilename().split("\\.");
            if (unknown.length > 1){
                format = unknown[unknown.length-1];
            }
        }
